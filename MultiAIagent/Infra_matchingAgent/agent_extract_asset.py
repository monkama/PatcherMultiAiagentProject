...
MAX_TOKENS = 4096
TEMPERATURE = 0.0
TOOL_OUTPUT_LIMIT = 2000  # Bedrock 에 돌려줄 tool 응답 최대 길이
                ...
                "properties": {
                    "installed_software": {
                        ...
                        "items": {
                            "type": "object",
                            "properties": {
                                "vendor":      {"type": "string", "description": "CPE 벤더명 (예: f5, apache)"},
                                "product":     {"type": "string", "description": "CPE 제품명 (예: nginx, log4j)"},
                                "version":     {"type": "string", "description": "설치된 버전 (예: 1.18.0)"},
                                "cpe":         {"type": "string", "description": "CPE 2.3 식별자"},
                                "source_path": {"type": "string", "description": "탐지 근거 경로 (선택)"},
                            },
                    ...
                    "network_context": {
                        ...
                        "properties": {
                            "public_ip":          {"type": "string", "description": "IMDS 상 퍼블릭 IPv4. 없으면 빈 문자열."},
                            "listening_ports":    {"type": "array", "description": "LISTEN 상태 포트 번호 목록", "items": {"type": "integer"}},
                            "is_internet_facing": {"type": "boolean", "description": "public_ip 유무로 결정"},
                        },
                    },
                    "security_context": {
                        "type": "object",
                        ...
                        "properties": {
                            "attached_iam_role": {"type": "string", "description": "EC2 인스턴스 프로파일 이름 (없으면 빈 문자열)"},
                            "running_as_root":   {"type": "array",  "description": "root 로 실행 중인 취약 서비스의 comm 이름", "items": {"type": "string"}},
                            "imds_v2_enforced":  {"type": "boolean", "description": "IMDSv2 강제 여부 (SSRF 방어 지표)"},
                            "selinux_enforced":  {"type": "boolean", "description": "SELinux Enforcing 여부"},
                        },
                    },
                    "data_classification": {
                        "type": "string",
    ...
    "toolSpec": {
        ...
        "inputSchema": {
            "json": {
                "type": "object",
                "properties": {
                    "answer":     {"type": "string", "description": "질문에 대한 짧고 명확한 답변"},
                    "evidence":   {"type": "array",  "description": "답변 근거가 된 명령어 출력 또는 파일 경로", "items": {"type": "string"}},
...
COLLECT_TOOL_CONFIG = {"tools": [RUN_COMMAND_TOOL, READ_FILE_TOOL, SAVE_RESULT_TOOL]}
QUERY_TOOL_CONFIG   = {"tools": [RUN_COMMAND_TOOL, READ_FILE_TOOL, ANSWER_QUERY_TOOL]}
...
# ---------------------------------------------------------------------------
# Bedrock Converse API 호출 래퍼 (Throttling/지연 백오프 재시도)
# ---------------------------------------------------------------------------

_RETRYABLE = {"ThrottlingException", "ServiceUnavailableException", "ModelTimeoutException"}


def _bedrock_converse(region: str, system_prompt: str, messages: list, tool_config: dict) -> dict:
    """Bedrock Converse API 호출.

    Returns: converse() 응답 dict.
    Throttling/일시 장애는 지수 백오프 재시도. 그 외는 즉시 raise.
    """
    bedrock = _client("bedrock-runtime", region)
    last_exc = None
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if code in _RETRYABLE and attempt < MAX_RETRIES - 1:
                ...
                print(f"[AGENT] Bedrock {code} — {wait}초 후 재시도 ({attempt + 1}/{MAX_RETRIES})")
                time.sleep(wait)
                continue
            raise
    raise RuntimeError(f"Bedrock 호출 실패: {last_exc}")
...
def run_collect_agent(payload: dict,
    ...
    """수집 모드 에이전트 루프 (Bedrock Converse). save_result payload dict 반환."""
    ...
    print(f"[AGENT] 수집 모드 시작 — 모델: {BEDROCK_MODEL_ID}, 실행: {exec_mode}, 최대 턴: {MAX_AGENT_TURNS}")

    recovery_attempted = False
    for turn in range(MAX_AGENT_TURNS):
        response = _bedrock_converse(region, system_prompt, messages, COLLECT_TOOL_CONFIG)
        output_msg = response["output"]["message"]
        stop_reason = response.get("stopReason", "")
        content_blocks = output_msg.get("content", []) or []

        if not content_blocks:
            print(f"[AGENT] 턴 {turn + 1}: 응답 비어 있음 (stop={stop_reason})")
            if recovery_attempted:
                print("[AGENT] 복구 재시도도 실패 — 종료")
                break
            recovery_attempted = True
            print("[AGENT] 복구 시도 — 지금까지 모은 정보로 save_result 호출 유도")
        ...
        # assistant 메시지 전체를 history 에 보존 (Claude 가 toolUseId 매칭 시 필요)
        ...
        if stop_reason != "tool_use" or not tool_uses:
            ...
            print(f"[AGENT] 턴 {turn + 1}: 텍스트 응답 (stop={stop_reason}) — {text[:200]}")
            break
...
def run_query_agent(asset_info: dict, query: str,
                    instance_id: str | None = None,
                    region: str = DEFAULT_REGION) -> dict:
    """질의 응답 모드 (Bedrock Converse). answer_query payload dict 반환."""
    system_prompt = build_query_system_prompt(asset_info)

    messages: list = [{
        "role": "user",
        "content": [{"text": f"[질문] {query}"}],
    }]
    ...
    for turn in range(MAX_AGENT_TURNS):
        response = _bedrock_converse(region, system_prompt, messages, QUERY_TOOL_CONFIG)
        output_msg = response["output"]["message"]
        stop_reason = response.get("stopReason", "")
        content_blocks = output_msg.get("content", []) or []
