...
_OPENAI_BASE_URL = str(os.environ.get("OPENAI_BASE_URL") or "").strip()
...
def _openai_model_id() -> str:
    return (os.getenv("OPENAI_MODEL") or os.getenv("OPENAI_MODEL_ID") or _DEFAULT_OPENAI_MODEL).strip()
...
def _require_strands() -> None:
    if Agent is None or OpenAIModel is None:
        raise RuntimeError(
            "strands-agents with OpenAI model support is required for vulnerability payload generation. "
...
def _run_structured_agent(
    *,
    ...
    if _OPENAI_BASE_URL:
        client_args["base_url"] = _OPENAI_BASE_URL
