...
- 프로젝트 루트 `.env`에 AWS 인증, Bedrock 모델 접근용 설정, 오케스트레이터 ARN이 있어야 합니다.
...
- 오케스트레이터와 각 에이전트는 현재 **Amazon Bedrock 기반**으로 동작합니다.
- 실행 전에는 사용하려는 Bedrock 모델에 대한 접근 권한이 준비되어 있어야 합니다.
