...
export function buildResultTimeline(result: unknown): TimelineStep[] {
  ...
  return [
    ...
    },
    {
      ...
      highlights: [
        { label: '전략 갯수', value: patchRecords.length ? `${patchRecords.length}건` : '-' },
        { label: '조치 분포', value: summarizeActionMix(patchRecords) },
      ],
...
function summarizeActionMix(records: Record<string, unknown>[]): string {
  ...
  const parts = [
    applyNow > 0 ? `즉시 ${applyNow}` : '',
    applyPlanned > 0 ? `계획 ${applyPlanned}` : '',
    manualMitigation > 0 ? `완화 ${manualMitigation}` : '',
    humanReview > 0 ? `검토 ${humanReview}` : '',
  ].filter(Boolean);
  ...
  for (const [instanceId, group] of groups.entries()) {
    ...
    sections.push({
      ...
      fields: [
        ...
        { label: '인스턴스 ID', value: instanceId },
      ],
      entries: group.entries.map((entry) => ({
        title: `${entry.cveId}${entry.title ? ` · ${entry.title}` : ''}`,
        meta: `기준 위험도 ${entry.baseRisk} | 조정 결과 ${entry.adjustedRisk}`,
        body: entry.reason,
      })),
    });
  }
