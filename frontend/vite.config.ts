    ...
    configureServer(server) {
      ...
      server.middlewares.use('/api/run-orchestrator', async (request, response) => {
        ...
        try {
          ...
          const blockReason = getBlockedRunReason(form, Boolean(body.confirmedDangerousRun));
          ...
          const bundle = runResult.bundle ?? await readLatestResultBundle(resultRoot);
          const summary = runResult.summary ?? await readJsonIfExists(resolve(resultRoot, 'orchestrator_agent/latest/summary.json'));
...
async function runOrchestrator(repoRoot: string, form: RunForm) {
  ...
  return new Promise<OrchestratorRunResult>((resolveRun, reject) => {
...
function buildInteractiveInput(form: RunForm): string {
  ...
  } else if (mode === 'patch_exec_only') {
    lines.push(form.patch_execution_runtime_arn || '', '', '');
...
function appendStopStageInputs(lines: string[], stopStage: string, form: RunForm) {
  if (stopStage === 'vuln') {
    ...
    lines.push('', '');
  } else if (stopStage === 'patch') {
    lines.push(form.patch_impact_runtime_arn || '', form.infra_matching_runtime_arn || '', '', '', '');
  } else if (stopStage === 'patch_execution') {
    lines.push(form.patch_execution_runtime_arn || '', '', '');
  }
}
...
function getBlockedRunReason(form: RunForm, confirmedDangerousRun: boolean): string {
  if (!requiresDangerousRunConfirmation(form)) return '';
  if (confirmedDangerousRun) return '';

  if (form.mode === 'full') return 'Full 실행은 실제 patch execution까지 포함될 수 있어 확인이 필요합니다.';
  if (form.mode === 'patch_exec_only') return 'Patch execution 실행은 실제 변경 가능성이 있어 확인이 필요합니다.';
  if (form.mode === 'test' && form.stop_stage === 'patch_execution') return 'Test 모드의 patch_execution 단계 실행은 확인이 필요합니다.';
  return '';
}

function requiresDangerousRunConfirmation(form: RunForm): boolean {
  return form.mode === 'full'
    || form.mode === 'patch_exec_only'
    || (form.mode === 'test' && form.stop_stage === 'patch_execution');
}
...
async function readLatestResultBundle(resultRoot: string) {
  ...
  const conversationRoot = resolve(process.cwd(), '../MultiAIagent/Conversationlog');
  try {
    const conversationEntries = await readdir(conversationRoot, { withFileTypes: true });
    for (const entry of conversationEntries) {
      if (!entry.isDirectory()) continue;

      const latestPath = resolve(conversationRoot, entry.name, 'latest.json');
      const relativePath = `MultiAIagent/Conversationlog/${entry.name}/latest.json`;
      const value = await readJsonIfExists(latestPath);
      if (value === null) continue;

      conversations[entry.name] = {
        path: relativePath,
        value,
      };
    }
  } catch {
    // Conversation logs may not exist yet.
  }
